...
@export var gems_requis: int = 5  # Seuil pour activer le portail
...
func collect_gem():
	gems_collected += 1
	emit_signal("gems_updated", gems_collected)  # Émet le signal avec le nombre de gemmes
	check_threshold()

# Vérifie si le seuil est atteint
func check_threshold():
	if gems_collected >= gems_requis and not portal_open:
		portal_open = true
