...
func _ready():
	var game_manager = get_node("/root/GameManager")  # Assurez-vous que le chemin est correct
	game_manager.connect("gems_updated", Callable(self, "_on_gems_updated"))

func _on_gems_updated(gems_collected: int):
	label_coins.text = "X" + str(gems_collected)  # Met à jour l'affichage des gemmes


#func _on_game_manager_gems_updated(gems_collected: int) -> void:
		#label_coins.text = "X" + str(gems_collected)  # Met à jour l'affichage des gemmes
